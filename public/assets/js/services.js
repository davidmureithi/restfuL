...
(function($) {
    ...
    $('#updateProfileBtn').click(function() {
        ...
        $.ajax({
            ...
            data: {
                ...
                'display_name': $('#up_display_name').val(),
    ...
    $('#searchAgentSubmit').click(function() { 
        ...
        $.ajax({
            ...
            url: ajaxURL,
            data: {
                'action': 'reales_search_agents',
            },
            success: function(data) {
                $('#listC').html(services_vars.send_message).removeClass('disabled');
                if(data.signedup === true) {
                    var message = '<div class="alert alert-success alert-dismissible fade in" role="alert">' +
                                      '<div class="icon"><span class="fa fa-check-circle"></span></div>' + data.message +
                                  '</div>';
                    $('#listcompany').modal('hide');
                    document.location.href="http://spotpata.co.ke/account-settings/";

                    $('#userMessageModal').modal('show').on('load.bs.modal', function(e) {
                        $('#userMessageModal .modal-dialog').removeClass('modal-sm');
                            $('#userMessage_response').html('thanks');
                            $('.close-modal').click(function() {
                                $('#userMessage_response').empty();
                                $('#userMessageModal').modal('hide');
                                $('#userMessageModal .modal-dialog').addClass('modal-sm');
                            });
                    });                    
                } else {
                    var message = '<div class="alert alert-danger alert-dismissible fade in" role="alert">' +
                                      '<div class="icon"><span class="fa fa-ban"></span></div>' + data.message +
                                  '</div>';
                    $('#listCMMessage').empty().append(message);
                }
            },
            error: function(errorThrown) {
    ...
    $('#getStarted').click(function() {
        ...
        var ajaxURL = services_vars.admin_url + 'admin-ajax.php';
        var security = $('#securityOffer').val();
        $('#Mregister_Response').empty();
        $('#Mregister_Response').html('<div class="propSaving"><span class="fa fa-spin fa-spinner"></span> ' + services_vars.signin_loading + ' </div>');
        ...
        $.ajax({
            ...
            url: ajaxURL,
            data: {
                'action': 'spotpata_save_agent_register',
                'new_id': $('#new_id').val(),
                'user': $('#current_user').val(),                
                'name': $('#agentName').val(), 
                'mobile': $('#agentMobile').val(),               
                'email': $('#agentEmail').val(),
                'identity': $('#agentIdentity').val(), 
                'location': $('#agentLocation').val(),               
                'message': $('#agentMessage').val(),
                'gender': $('#agentGender').val(), 
                'category': $('#agentServiceCategory').val(),            
                'security': security
            },
            success: function(data) {
                var message = '';
                if(data.save === true) {
                    $('#new_id').val(data.reqID);
                    //$('#submitRequestBtn').html('<span class="fa fa-save"></span> ' + services_vars.update_property);
                    if(data.reqStatus == 'publish') {
                        $('#viewRequestBtn').css('display', 'inline-block').attr('href', data.reqLink);
                    }
                    $('#deleteRequestBtn').css('display', 'inline-block');
                ...
                $('#MregisterModal .modal-dialog').removeClass('modal-sm');
                $('#Mregister_Response').html(message);
                $('.close-modal').click(function() {
                    $('#Mregister_Response').empty();
                    $('#MregisterModal').modal('hide');
                    $('#offerMaterial').modal('hide');
                    $('#MregisterModal .modal-dialog').addClass('modal-sm');
                });
            },
            error: function(errorThrown) {
    ...
    $('#getDone').click(function() {
        ...
        var security = $('#securityUserRequest').val();
        $('#Mrequest_Response').empty();
        $('#Mrequest_Response').html('<div class="propSaving"><span class="fa fa-spin fa-spinner"></span> ' + services_vars.signin_loading + ' </div>');
        ...
        $.ajax({
            ...
            url: ajaxURL,
            data: {
                'action': 'spotpata_save_user_request',
                'new_id': $('#new_id').val(),
                'user': $('#current_user').val(),
                ...
                'rwhen': $('#userPickedDay').val(),
                'location': $('#userLocation').val(),
                'request': $('#userRequest').val(),
                'callme': $('#checkboxCallMe').is(':checked'),
                'rcategory': $('input[name=category]:checked').val(),
                'gender': gender,
                'security': security
            },
            success: function(data) {
                ...
                if(data.save === true) {
                    ...
                    message =   '<div class="alert alert-success alert-dismissible fade in" role="alert">' +
                                    ...
                                    data.message +
                                '</div>';
                    document.location.href="http://spotpata.co.ke/order-received";
                } else {
                    message =   '<div class="alert alert-danger alert-dismissible fade in" role="alert">' +
                                    '<div class="icon"><span class="fa fa-ban"></span></div>' +
                                    '<button type="button" class="close close-modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button>' +
                ...
                $('#MrequestModal .modal-dialog').removeClass('modal-sm');
                $('#Mrequest_Response').html(message);
                $('.close-modal').click(function() {
                    $('#Mrequest_Response').empty();
                    $('#Mrequest_Modal').modal('hide');
                    $('#detailedMaterial').modal('hide');
                    $('#Mrequest_Modal .modal-dialog').addClass('modal-sm');
                });
            },
            error: function(errorThrown) {
            }
    ...
    //Subscribe and Contact our Representative
    $('#subscribeC').click(function() {
        $('#Mrequest_Modal').modal({
            'backdrop': true,
            'keyboard': false,
            'show': true
